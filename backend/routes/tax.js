/* —————————————————————————————————————
 * Tax Routes
 * Full tax module: profiles, estimates, tagging, payments,
 * documents, reports, and an admin seeding endpoint.
 *
 * Key behaviors:
 *   - Feature-gated by FEATURE_TAX_MODULE env var. When disabled,
 *     every non-admin route returns 404.
 *   - Auth is required; admin routes additionally require `admin`.
 *   - Per-user rate limiters (userLimiter) key on the authenticated
 *     user ID, not IP.
 *   - Audit logs every data access via `audit()`.
 *   - Tax document `file_url` is encrypted at rest (AES-256-GCM).
 *   - Optimistic concurrency on profiles via `_version`.
 *   - All numeric/list validation uses express-validator.
 * ————————————————————————————————————— */
...
// ── Jurisdictions supported by the tax engine ──
const supportedJurisdictions = new Set(['IN', 'US']);

// ── Treatments that count as deductible for duplicate-deduction checks ──
const deductibleTreatments = new Set([
  'deductible', 'business_expense', 'medical', 'charity', 'retirement', 'education',
]);
...
// ── Build a per-user rate limiter ──
// Keys on the authenticated user ID, falling back to 'anonymous'.
...
// ── Return 400 with the first validation error, or true if valid ──
const resultOrBadRequest = (req, res) => {
  const result = validationResult(req);
  if (!result.isEmpty()) {
    res.status(400).json({
      error: result.array()[0].msg,
      code: 'TAX_VALIDATION_ERROR',
      errors: result.array(),
    });
    return false;
  }
...
// ── Parse a monetary value from request input ──
// Returns:
//   - undefined when optional and absent
//   - null when required and absent, or when invalid
//   - a two-decimal number otherwise
...
// India: April 1 → next April 1. US: Jan 1 → next Jan 1.
const fiscalBounds = (profile) => {
  ...
  if (profile.jurisdiction === 'IN') {
    return {
      ...
      end: new Date(`${year + 1}-04-01T00:00:00.000Z`),
    };
  }
  return {
...
// ── Emit a structured audit log entry for tax data access ──
...
// ── Load a tax profile owned by the authenticated user ──
const findOwnedProfile = async (req, id) =>
  TaxProfile.findOne({ _id: id, user_id: userIdOf(req) });

// ── Load the rule set matching a profile's jurisdiction/year/regime ──
...
// ── Respond with a clear error when no matching rule set exists ──
...
// ── Load the user's transactions within the profile's fiscal year ──
...
// ── Load the user's tax-tagged transactions, joined with the tag row ──
// Optionally scoped to a profile and/or a date range.
const getTaggedTransactions = async (req, profileId, start, end) => {
  ...
  return tags
    .map((tag) => ({ tag, transaction: byId.get(String(tag.transaction_id)) }))
    .filter((item) => item.transaction);
};

/* ── Document encryption ──────────────────────────────────── */

// ── Derive the AES-256-GCM key from the env-provided secret ──
const getCryptoKey = () => {
  const raw = String(process.env.TAX_FIELD_ENCRYPTION_KEY || '').trim();
  if (!raw) {
    throw Object.assign(
      new Error('Tax document encryption is not configured.'),
      { status: 503, code: 'TAX_ENCRYPTION_NOT_CONFIGURED' }
    );
  }
  return crypto.createHash('sha256').update(raw).digest();
};

// ── Encrypt a string into iv.tag.ciphertext (base64url encoded) ──
const encrypt = (value) => {
...
// ── Decrypt the iv.tag.ciphertext format produced by `encrypt` ──
const decrypt = (value) => {
...
// ── Prepare a document for API output (decrypts file_url) ──
const serializeDocument = (doc) => {
  const plain = { ...doc };
  try {
    plain.file_url = decrypt(doc.file_url);
  } catch {
    plain.file_url = null;
  }
  return plain;
...
// ── Cache-Control + feature gate (admin routes bypass the gate) ──
router.use((req, res, next) => {
  res.set('Cache-Control', 'no-store, no-cache, must-revalidate, private');
  if (!isEnabled() && !req.path.startsWith('/admin/')) {
    return res.status(404).json({ error: 'Tax module is not enabled.' });
  }
  return next();
});

// ── Auth + general rate limit for all tax routes ──
router.use(auth);
...
// ── GET /profiles
// List the user's tax profiles, excluding legacy/incomplete records.
router.get('/profiles', async (req, res) => {
  // Ignore legacy/incomplete records so the UI never renders a profile
  // with undefined jurisdiction or fiscal year. They remain untouched
  // for audit.
...
router.post('/profiles', [
  ...
  body('tax_id_last4').optional({ checkFalsy: true, nullable: true })
    .matches(/^[A-Za-z0-9]{4}$/)
    .withMessage('Only the last four tax-ID characters may be stored.'),
], async (req, res) => {
  // ── Validate input ──
  if (!resultOrBadRequest(req, res)) return;

  // ── Check for an existing profile for the same fiscal year ──
  const duplicate = await TaxProfile.exists({
    user_id: userIdOf(req),
    fiscal_year: Number(req.body.fiscal_year),
  });
  ...
  const profile = await TaxProfile.create({
    ...
    tax_id_last4: req.body.tax_id_last4
      ? String(req.body.tax_id_last4).toUpperCase()
      : null,
  ...
  return res.status(201).json({
    profile,
    warning: duplicate ? 'Another profile already exists for this fiscal year.' : null,
    message: 'Tax profile created.',
  });
});

// ── GET /profiles/:id
// Fetch a single owned tax profile.
...
// ── PUT /profiles/:id
// Update a profile with optimistic concurrency via `_version`.
router.put('/profiles/:id', [idParam], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  // ── Whitelist updatable fields ──
  const allowed = [
    'name', 'jurisdiction', 'fiscal_year', 'filing_status', 'tax_regime',
    'dependents', 'residency_status', 'currency', 'pre_tax_contributions',
    'itemized_deductions', 'tax_credits', 'capital_loss_carryforward',
    'shared_with_household', 'tax_id_last4', 'notes', 'is_active',
  ];
  const updates = Object.fromEntries(
    Object.entries(req.body).filter(([key]) => allowed.includes(key))
  );

  // ── Normalize / validate specific fields ──
  if (updates.tax_id_last4 === '') updates.tax_id_last4 = null;
  if (updates.jurisdiction && !supportedJurisdictions.has(String(updates.jurisdiction))) {
    return res.status(400).json({ error: 'Unsupported tax jurisdiction.' });
  }
  if (updates.tax_id_last4 && !/^[A-Za-z0-9]{4}$/.test(String(updates.tax_id_last4))) {
    return res.status(400).json({ error: 'Only the last four tax-ID characters may be stored.' });
  }

  // ── Apply the update (guarded by `_version` when provided) ──
  const profile = await TaxProfile.findOneAndUpdate(
    {
      _id: req.params.id,
      user_id: userIdOf(req),
      ...(req.body._version !== undefined ? { _version: Number(req.body._version) } : {}),
    },
    {
      $set: {
        ...updates,
        ...(updates.currency ? { currency: String(updates.currency).toUpperCase() } : {}),
      },
      $inc: { _version: 1 },
    },
  ...
  // ── Reject on version conflict ──
  if (!profile) {
    return res.status(409).json({
      error: 'Profile changed in another tab. Refresh and try again.',
      code: 'TAX_PROFILE_CONFLICT',
    });
  }
...
// ── DELETE /profiles/:id
// Delete a profile and all associated tax records.
router.delete('/profiles/:id', [idParam], async (req, res) => {
  ...
  // ── Delete tag/payment/document records and the profile itself ──
  await Promise.all([
...
// ── Shared estimate logic, called by /estimate ──
const estimate = async (req, res) => {
  ...
  // ── Require a matching rule set ──
  const ruleSet = await getRuleSet(profile);
  if (!ruleSet) return missingRules(res, profile);

  // ── Load fiscal transactions and optional income override ──
  ...
  if (req.body.income_override !== undefined) {
    ...
    if (incomeOverride === null) {
      return res.status(400).json({
        error: 'Income cannot be negative or malformed.',
        code: 'TAX_NEGATIVE_INCOME',
      });
    }
  }
  const incomeTransactions = incomeOverride == null
    ? transactions
    : [{ type: 'income', amount: incomeOverride }];

  // ── Load supporting data ──
  const tagged = await getTaggedTransactions(
    req, profile._id, fiscalBounds(profile).start, fiscalBounds(profile).end
  );
  const payments = await TaxPayment.find({
    user_id: userIdOf(req),
    tax_profile_id: profile._id,
  }).lean();
  const holdings = await WealthItem.find({
    user_id: userIdOf(req),
    sold_at: { $ne: null },
  }).lean();

  // ── Run the tax engine ──
  const result = estimateAnnualTax({
  ...
  if (!result) {
    return res.status(400).json({ error: 'Tax data is incomplete. Verify the profile and rule set.' });
  }
  ...
  return res.json({
    ...
    ruleSet: {
      ...
      fiscal_year: ruleSet.fiscal_year,
      regime: ruleSet.regime,
      currency: ruleSet.currency,
      brackets: ruleSet.brackets,
      standard_deduction: ruleSet.standard_deduction,
      advance_tax_dates: ruleSet.advance_tax_dates,
      advance_tax_schedule: ruleSet.advance_tax_schedule,
...
// ── POST /estimate
// Run a tax estimate for a profile.
router.post('/estimate', estimateLimiter, [profileIdBody], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;
  try {
    return await estimate(req, res);
  } catch (error) {
    logger.error('Tax estimate failed', error);
    return res.status(error.status || 500).json({ error: error.message || 'Tax estimate failed.' });
  }
});

// ── POST /estimate/compare
// Compare new vs. old regime for Indian profiles.
router.post('/estimate/compare', estimateLimiter, [profileIdBody], async (req, res) => {
  ...
  if (profile.jurisdiction !== 'IN') {
    return res.status(400).json({ error: 'Regime comparison is currently available for India profiles only.' });
  }
  ...
  // ── Run the estimate under each regime ──
  const comparison = {};
  for (const regime of ['new', 'old']) {
    const ruleSet = await TaxRuleSet.findOne({
      jurisdiction: 'IN',
      fiscal_year: profile.fiscal_year,
      regime,
    }).lean();
    if (!ruleSet) return missingRules(res, { ...profile.toObject(), jurisdiction: 'IN' });
    comparison[regime] = estimateAnnualTax({
      profile: { ...profile.toObject(), tax_regime: regime },
      ruleSet,
      transactions,
      taggedTransactions: tagged,
      payments,
...
// ── POST /estimate/capital-gains
// Estimate capital gains tax from sold holdings.
router.post('/estimate/capital-gains', gainsLimiter, [profileIdBody], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  // ── Load the profile and matching rule set ──
  const profile = await findOwnedProfile(req, req.body.profile_id);
  if (!profile) {
    return res.status(404).json({
      error: 'Tax profile not found. Create one first.',
  ...
  // ── Load sold holdings and run the calculation ──
  const holdings = await WealthItem.find({
    user_id: userIdOf(req),
    sold_at: { $ne: null },
  }).lean();
...
// ── POST /tag
// Create or update a tax tag for a transaction.
router.post('/tag', tagLimiter, [
  body('transaction_id').isMongoId(),
  profileIdBody,
  body('treatment').isIn([
    'deductible', 'non_deductible', 'capital_gain', 'capital_loss', 'exempt',
    'business_expense', 'medical', 'charity', 'retirement', 'education',
  ]),
  ...
  if (
    ...
    (deductibleTreatments.has(existing.treatment) || deductibleTreatments.has(req.body.treatment))
  ) {
    return res.status(409).json({
      error: `This transaction is already claimed under ${existing.treatment}. Remove the other tag first.`,
      code: 'TAX_DUPLICATE_DEDUCTION',
    });
  }
...
// ── DELETE /tag/:transaction_id
// Remove the tax tag for a transaction.
router.delete('/tag/:transaction_id', [param('transaction_id').isMongoId()], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  await TaxTag.deleteOne({
...
// List tax-tagged transactions, optionally filtered by profile and treatment.
router.get('/tagged-transactions', [query('profile_id').optional().isMongoId()], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  const rows = await getTaggedTransactions(
    req, req.query.profile_id, req.query.start, req.query.end
  );
...
// ── DELETE /payments/:id
// Remove a tax payment.
router.delete('/payments/:id', [idParam], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  const deleted = await TaxPayment.findOneAndDelete({
    _id: req.params.id,
    user_id: userIdOf(req),
  });
...
// ── GET /documents
// List tax documents with `file_url` decrypted for display.
...
// ── POST /documents
// Index a new tax document (file_url is encrypted at rest).
router.post('/documents', [
  ...
  // ── Create the document with an encrypted file_url ──
  const document = await TaxDocument.create({
    ...req.body,
    user_id: userIdOf(req),
    file_url: encrypt(req.body.file_url),
  });

  audit(req, 'document_create', profile._id);
  return res.status(201).json({
    document: serializeDocument(document.toObject()),
    message: 'Tax document indexed.',
  });
});
...
router.delete('/documents/:id', [idParam], async (req, res) => {
  ...
  const deleted = await TaxDocument.findOneAndDelete({
    _id: req.params.id,
    user_id: userIdOf(req),
  });
...
// Generate a tax report (json default, csv optional, pdf client-side).
router.get('/report/:profile_id', reportLimiter, [
  param('profile_id').isMongoId(),
  query('format').optional().isIn(['json', 'csv', 'pdf']),
], async (req, res) => {
  if (!resultOrBadRequest(req, res)) return;

  // ── Load the profile and rule set ──
  ...
  // ── Compute the estimate and sales-tax summary ──
  const estimateResult = estimateAnnualTax({
    profile: profile.toObject(),
    ruleSet,
    transactions,
    taggedTransactions: tagged,
  ...
  const salesTax = calculateSalesTaxSummary({
    transactions,
    defaultRate: ruleSet.sales_tax_default,
  });
  ...
  // ── PDF is handled client-side via browser print ──
  if (format === 'pdf') {
    return res.status(501).json({
      error: 'PDF export is available through the browser print-to-PDF view.',
      code: 'TAX_PDF_CLIENT_EXPORT',
    });
  }

  // ── CSV output ──
  if (format === 'csv') {
    // Prefix formula-triggering characters so spreadsheets treat them
    // as text rather than evaluating them.
    const escape = (value) => {
...
// Idempotently seed tax rule sets for supported jurisdictions.
